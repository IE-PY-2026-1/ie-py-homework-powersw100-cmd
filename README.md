...
원인: input()은 모든 입력을 문자열로 받기 때문에 제곱 연산(**2)이나 나눗셈이 불가능했음.
해결: float() 함수를 사용하여 입력값을 실수형으로 형변환하여 계산 로직을 정상화함.

문제 2: 조건문 범위 설정 오류
원인: if BMI < 18.5와 같은 조건식에서 부등호 방향이나 콜론(:) 누락으로 구문 오류 발생.
해결: 파이썬 특유의 들여쓰기(Indentation) 규칙과 조건문 끝의 : 사용법을 다시 확인하여 수정함.
